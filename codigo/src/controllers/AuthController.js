...
class AuthController {
  ...
  async login(req, res) {
    try {
      ...
      if (role === 'empresa') {
        ...
        const ok = hash && hash.startsWith('$2') ? await bcrypt.compare(senha, hash) : (senha === hash);
      ...
      if (role === 'aluno') {
        ...
        // Preferência: email/senha. Fallback: cpf/rg (mantém compatibilidade)
        if (email && senha) {
          // Usa o adapter via empresaDao.db para consultar 'usuarios'
          const db = this.empresaDao.db;
          const usuarios = await db.findAll('usuarios', { email });
          const usuario = usuarios && usuarios[0];
          if (!usuario) return res.status(401).json({ error: 'Credenciais inválidas' });
          const ok = usuario.senha_hash && usuario.senha_hash.startsWith('$2')
        ...
        return res.status(400).json({ error: 'Informe email/senha ou cpf/rg' });
      }

      return res.status(400).json({ error: 'role inválida. Use "aluno" ou "empresa"' });
    } catch (err) {
  ...
  async registerAluno(req, res) {
    try {
      const { nome, cpf, rg, endereco, curso, email, senha } = req.body || {};
      if (!nome || !cpf || !rg || !endereco || !curso || !email || !senha) {
        return res.status(400).json({ error: 'Campos obrigatórios ausentes' });
      }
      const db = this.empresaDao.db;
      const jaExiste = await db.findAll('usuarios', { email });
      if (jaExiste && jaExiste[0]) return res.status(409).json({ error: 'Email já cadastrado' });
      const senha_hash = await bcrypt.hash(senha, 10);
      const usuario = await db.insert('usuarios', { email, senha_hash, ativo: true, saldo: 0 });
      // cria aluno apontando para mesmo id do usuario
      const aluno = await db.insert('alunos', { id: usuario.id, nome, cpf, rg, endereco, curso });
      const token = this.sign({ id: usuario.id }, 'aluno');
      return res.status(201).json({ token, role: 'aluno', aluno });
    } catch (err) {
      return res.status(500).json({ error: err.message });
    }
  }
